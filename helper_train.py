...
# Use this
def train_vae_v2(num_epochs, model, optimizer, device, 
                 train_loader, valid_loader=None,
                 loss_fn=None,
                 logging_interval=100, 
                 skip_epoch_stats=False,
    ...
    log_dict = {'train_combined_loss_per_batch': [],
                ...
                'train_acc_per_epoch': [],       # Add this
                'valid_acc_per_epoch': [],       # Add this
                'valid_loss_per_epoch': []}      # Add this
    ...
    for epoch in range(num_epochs):
        ...
        if not skip_epoch_stats:
            ...
            with torch.set_grad_enabled(False):  # save memory during inference
                ...
                train_acc = compute_accuracy(model, train_loader, device)         # Add this
                train_loss = compute_epoch_loss_autoencoder(
                    model, train_loader, loss_fn, device)
                print('***Epoch: %03d/%03d | Loss: %.3f' % (
                      epoch+1, num_epochs, train_acc, train_loss))                # Add train_acc
                log_dict['train_combined_per_epoch'].append(train_loss.item())
                log_dict['train_acc_per_epoch'].append(train_acc.item())          # Add this
                
                # Add this (valid_loader)
                if valid_loader is not None:
                    valid_acc = compute_accuracy(model, valid_loader, device)
                    valid_loss = compute_epoch_loss_classifier(
                        model, valid_loader, loss_fn, device)
