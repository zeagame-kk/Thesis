...
def train_vae_v1(num_epochs, model, optimizer, device, 
    ...
    for epoch in range(num_epochs):
        ...
        model.train()
        for batch_idx, (features, _) in enumerate(train_loader):    # `_` actually is target but, now this is unsupervised. `_` is a convention that we don't use a variable.
            ...
            # Compute pixel wise term
            pixelwise = loss_fn(decoded, features, reduction='none')     # this is our reconstruction loss!!!    # `features` here represents to inputs
            pixelwise = pixelwise.view(batchsize, -1).sum(axis=1) # sum over pixels   # view=reshape so, the size should (batchsize, vector)
